...
def bottleneck_block( input, num_filters, stride, cardinality, name):
        ...
        conv0 = conv_bn_layer(input=input,num_filters=num_filters,filter_size=1,act='relu',name=name + "_branch2a")
        
        conv1_3 =conv_bn_layer(input=conv0,num_filters=num_filters,filter_size=3,padding=1,stride=stride,
                               groups=cardinality,act='relu',name=name + "_branch2b_1")
       
        conv1_1 =conv_bn_layer(input=conv0,num_filters=num_filters,filter_size=(3,1),padding=(1,0),stride=stride,

                               groups=cardinality,act='relu',name=name + "_branch2b_2")
        conv1_2 =conv_bn_layer(input=conv0,num_filters=num_filters,filter_size=(1,3),padding=(0,1),stride=stride,
                               groups=cardinality,act='relu',name=name + "_branch2b_3")
        ...
        conv2 = conv_bn_layer(input=conv1,num_filters=num_filters if cardinality == 64 else num_filters * 2,
...
def net(input, class_dim=1000):
        ...
        conv = conv_bn_layer(input=input,num_filters=64,filter_size=7,padding=3,stride=2, act='relu',name="res_conv1")  #debug
